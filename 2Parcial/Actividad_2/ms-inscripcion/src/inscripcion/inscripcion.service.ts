...
export class InscripcionService {
  ...
  async createInscripcion(data: {
    ...
    try {
      ...
      // 🔒 IDEMPOTENCIA: Si falla por duplicado, retornar la existente
      if (error.code === '23505' && error.constraint?.includes('message_id')) {
        console.log('⚠️  IDEMPOTENCIA: message_id duplicado detectado en BD');
        const existing = await this.repo.findOne({ where: { message_id: messageId } });
        if (existing) {
          console.log(`   ✅ Retornando inscripción existente: ${existing.id}`);
          return { inscripcion: existing, isNew: false };
    ...
    // 🌐 ACTIVIDAD 2: Publicar webhook cuando se crea una inscripción
